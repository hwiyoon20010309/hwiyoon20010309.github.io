...
* **문제** : 경쟁사 채용 현황을 수동으로 확인해야 해 시장 흐름 파악이 어려웠고, 직무별 인재 유출·수급 난이도를 알 방법이 없었습니다. 공고 작성 기준은 개인 경험에 의존해 품질이 들쭉날쭉했고, 채용 타이밍도 감각과 관행에만 의존했습니다.
* **해결** : 3일 주기로 경쟁사·IT 기업 공고를 자동 수집해 LLM Few-shot 프롬프트로 구조화하고, Jaccard + SBERT + Personalized PageRank를 결합한 Hybrid 알고리즘으로 직무를 매칭했습니다. LangGraph 기반 Multi-Agent가 공고 품질을 평가·개선 제안하고, React 대시보드로 트렌드·Retention 지표·채용 일정 시뮬레이션을 한 화면에 모았습니다.
* **성과** : 경쟁사 채용 동향과 집중 스킬을 대시보드 하나로 자동 모니터링할 수 있게 됐고, YoY 증가율 + HHI 기반의 설명 가능한 채용 난이도 지표로 객관적 의사결정을 지원합니다. 복합 인덱스·집계 테이블·OLTP/OLAP 분리로 데이터가 누적돼도 안정적으로 확장 가능한 아키텍처를 확보했습니다.
...
![SpeedJobs 서비스 아키텍처](/images/portfolio/speedjobs-architecture.png)
